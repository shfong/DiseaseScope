...
class DDOT_Client(object): 
    ...
    def get_output_file(self, filename="ddot_output.txt", create_object=False): 
        if not hasattr(self, "request"): 
            raise RuntimeError("No request object found. Job may not have been submitted yet.")

        r = requests.get(f"{self.location}/rawclusteringoutput")
        ...
        if create_object: 
            self.create_ddot_object(filename=self.output_filename)
